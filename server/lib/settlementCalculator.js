...
export async function generateSettlement(db, driverId, periodStart, periodEnd, createdBy) {
  ...
  const deductions = await calculateDeductions(db, driverId);
  ...
  await db('settlement_line_items').insert(itemRows);

  // Link loads to settlement
  await db('loads').whereIn('id', loads.map(l => l.id)).update({ settlement_id: settlement.id });
