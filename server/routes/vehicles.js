...
export default function vehiclesRouter(db) {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    let query = db('vehicles')
      .leftJoin('drivers', 'vehicles.current_driver_id', 'drivers.id')
      .select('vehicles.*', 'drivers.full_name as driver_name')
  ...
  router.patch('/:id', asyncHandler(async (req, res) => {
    ...
    const allowed = ['unit_number', 'type', 'vin', 'year', 'make', 'model', 'license_plate', 'license_state', 'status', 'current_driver_id', 'odometer', 'notes'];
    const updates = {};
  ...
  router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    await db('vehicles').where({ id: req.params.id }).update({ status: 'INACTIVE', current_driver_id: null, updated_at: db.fn.now() });
  ...
  router.post('/:id/assign-driver', asyncHandler(async (req, res) => {
    ...
    if (driver_id) {
      // Unassign this driver from any other vehicle of the same type
      await db('vehicles')
